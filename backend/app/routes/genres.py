...
# GET /genres/  — publiczny, potrzebny do filtrów na froncie
...
def get_genre(genre_id):
    ...
    if not genre:
        return jsonify({"error": "Gatunek nie został znaleziony"}), 404
...
def add_genre():
    try:
        ...
        return jsonify({"error": "Błąd podczas dodawania gatunku"}), 500
...
def remove_genre(genre_id):
    if delete_genre(genre_id):
        return jsonify({"message": "Gatunek usunięty"}), 200
    return jsonify({"error": "Gatunek nie został znaleziony"}), 404
